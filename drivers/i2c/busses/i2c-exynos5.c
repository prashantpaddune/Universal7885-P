...
#define HSI2C_MSTBUSY_HWACG			(1u << 26)
...
#define HSI2C_USI0_SYSREG		0x10032000
...
static int exynos5_i2c_suspend_noirq(struct device *dev)
{
	...
	dev_err(dev, "exynos5_i2c_suspend_noirq");
	...
	if(!(i2c->support_hsi2c_batcher)) {
		dev_err(i2c->dev, "###### 1 CTL 0x%08x\n", readl(i2c->regs + HSI2C_CTL));
		writel((HSI2C_SW_RST | HSI2C_MSTBUSY_HWACG), i2c->regs + HSI2C_CTL);
		dev_err(i2c->dev, "###### 2 CTL 0x%08x\n", readl(i2c->regs + HSI2C_CTL));
	}
	clk_disable(i2c->clk);
	exynos_update_ip_idle_status(i2c->idle_ip_index, 1);
#endif
...
static int exynos5_i2c_resume_noirq(struct device *dev)
{
	...
	dev_err(dev, "exynos5_i2c_resume_noirq");
	i2c_lock_adapter(&i2c->adap);
