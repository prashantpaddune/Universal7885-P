...
static struct fimc_is_sensor_cfg config_module_4ha_4lane[] = {
	...
	/* 2448x2448_30fps */
	FIMC_IS_SENSOR_CFG(2448, 2448, 30, 20, 3, CSI_DATA_LANES_4),
	/* 1632x1224_30fps */
	FIMC_IS_SENSOR_CFG(1632, 1224, 30, 20, 4, CSI_DATA_LANES_4),
	/* 1632x1224_60fps */
	FIMC_IS_SENSOR_CFG(1632, 1224, 60, 20, 5, CSI_DATA_LANES_4),
	/* 800x600_120fps */
	FIMC_IS_SENSOR_CFG(800, 600, 120, 20, 6, CSI_DATA_LANES_4),
};
	...
	struct exynos_platform_fimc_is_module *pdata)
{
	...
	if (exist_actuator == true) {
		if (gpio_is_valid(gpio_camaf_2p8_en)) {
			...
			SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, gpio_none, "VDDAF_2.8V_CAM", PIN_REGULATOR, 1, 1500);
		}
	} 
	...
	if (gpio_is_valid(gpio_cam_a2p8_en)) {
		SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, gpio_cam_a2p8_en, "FCAM_AVDD_2P8", PIN_OUTPUT, 0, 0);
	}
	...
	if (exist_actuator == true) {
		if (gpio_is_valid(gpio_camaf_2p8_en)) {
			...
			SET_PIN(pdata, SENSOR_SCENARIO_READ_ROM, GPIO_SCENARIO_OFF, gpio_none, "VDDAF_2.8V_CAM", PIN_REGULATOR, 1, 1500);
		}
	}
