...
static int hx_gap_hor_raw(int test_type, uint32_t * raw, int *result_raw)
{
	...
	for (i = 0; i < tx_num*rx_num; i++) {
		...
		*/
		...
		now_part = (i % rx_num) / g_gap_horizontal_partial;
		if (now_part % 2 == 0) {
		/*even part
		...
		/*odd part
			...
			if ((i % rx_num) == (g_gap_horizontal_part[now_part] + rx_num / g_gap_horizontal_partial)) {
				skip_flag = 1;
			}
		}
		if (skip_flag == 1) {
...
static int hx_gap_ver_raw(int test_type, uint32_t * org_raw, int *result_raw)
{
	...
	for (i = 0; i < tx_num * rx_num; i++) {
		...
		*/
		/* the now part will be known in odd or even */
		now_part = (i / rx_num) / g_gap_vertical_partial;
		if (now_part % 2 == 1) { 
		/*odd part
			using index i to know is is in which TX 
			if i is in the starting line of odd part it will need to skip*/
			if ((i / rx_num) == (g_gap_vertical_part[now_part] + (tx_num / g_gap_vertical_partial)))
				skip_flag = 1;
		} else { 
		/*even part
			using index i to know is is in which TX 
